...
function hide(prohibition) {
    ...
    while (count < blocks.length) {
        try {
            var nickname_block = blocks[count].children[0].children[1].children[0];
            nickname = nickname_block.textContent;
        } catch (e) {
        ...
        if (prohibition.name != []) {
            for (var i = 0; i < prohibition.name.length; i++) {
                if (nickname.includes(prohibition.name[i])) {
